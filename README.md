...
Colab（Google Colaboratory）を使うことを想定しています。「回帰分析すること」だけが目標のため、内容はかなり限定的です（Python/Rの包括的入門ではありません）。

補足：作成者（矢野）はPythonよりもRの方が（比較的）得意であるため、Pythonの更新は滞りがちです（履修生の皆さんは好きな言語を選んでください）。
