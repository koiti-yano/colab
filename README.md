...
このサイトでは、駒澤大学経済学部開講「経済統計a/b」履修生向けにPython/Rで回帰分析する方法を解説します。Colab（Google Colaboratory）を使うことを想定しています。なお、作成者（矢野）はPythonよりもRの方が得意（比較の問題ですが）であるため、Pythonの更新は滞りがちです（履修生の皆さんは好きな言語を選んでください）。
